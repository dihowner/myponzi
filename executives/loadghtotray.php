...
$loadGH = $con->prepare("select * from gethelp where merge!='YES' and merge!='Cancelled'");
$loadGH->execute();
// $sumblc = 0;
?>
<table border='1' width='30%' class='table'>
	<tr>
		<td style='color: #000'>Particpant</td>
		<td style='color: #000'>AMountGH</td>
		<td style='color: #000'>Balance</td>
	</tr>
	<tr>
<?php
for($i=1; $i<=rows("select * from gethelp where merge!='YES' and merge!='Cancelled'"); $i++)
{
	$loadGH_info =  $loadGH->fetch(PDO::FETCH_ASSOC);
	$amountGH = $loadGH_info['amountGH'];
	$balance = $loadGH_info['balance'];
	$merge = $loadGH_info['merge'];
	$participantID = $loadGH_info['participantID'];
	...
	// $sumblc += $balance;
	// echo 'Amount ' . $amountGH . ' Balance:' . $balance . ' merge:' . $merge . ' participantID:' . $participantID . '<br>';
	?>
		
		<td style='color: #000'><?php echo $participant_name;?></td>
		<td style='color: #000'><?php echo number_format($amountGH);?></td>
		<td style='color: #000'><?php echo number_format($balance);?></td>
	</tr>
	<?php
}
?>
