     ...
     <!-- Navigation -->
		...
		<div class="col_1">
			...
			<div class="col-md-12 stats-info">
               <br><font size='5px' color='red'><center><b>.::: EDIT PENDING GH QUEUE :::. </b></center></font>
				
				<?php
				if(isset($_GET['msg']))
				{
					$msg = $_GET['msg'];
					if($msg == strtoupper(substr(md5("Edited Successfully"), -4)))
					{
					?>
						<div class='alert alert-success'><b>Edited Successfully</b></div>
					<?php
					}					
					else if($msg == strtoupper(substr(md5("Error Occured"), -4)))
					{
					?>
						<div class='alert alert-success'><b>Error Occured</b></div>
					<?php
					}
				}
				?>
				...
				<form method='post' action='../action'>
				   <label><b>Select GH ID: </b></label>
				   
					<select class='form-control input-lg' name='GHIDQueue' id='GHIDQueue' required>
					<option value=''>Please select GH ID</option>
					<?php
					$getGHpending = $con->prepare("SELECT * FROM `gethelp` where merge='pending' order by ghID ASC");
					$getGHpending->execute();
					for($i=1; $i<=rows("SELECT * FROM `gethelp` where merge='pending' order by ghID ASC"); $i++)
					{
						$getGHpending_info = $getGHpending->fetch(PDO::FETCH_ASSOC);
						$pend_ghID = $getGHpending_info['ghID'];
						// $participant_id = $getGHpending_info['pid'];
					?>
						<option value='<?php echo $pend_ghID;?>'><?php echo ucfirst($pend_ghID);?></option>
					<?php
					}
					?>
					</select>
				   ...
				   <br>
				   <label><b>Enter Balance</b></label>
				   <input class='form-control input-lg' name='amount_blc' autocomplete='off' required>
				   <br>
					<select class='form-control input-lg' name='merge_status' id='merge_status' required>
						<option value=''>Choose Merge Status</option>
						<option value='pending'>pending</option>
						<option value='YES'>YES</option>
					</select>
					<br>
					<button class='btn btn-default' name='editpendingQueue' id='editpendingQueue'>Edit Pending GH Queue &raquo;</button><br><br><br>
				</form>
				
				
				<br><font size='5px' color='red'><center><b>.::: EDIT NEW GH QUEUE :::. </b></center></font>
				
				
				<form method='post' action='../action'>
				   <label><b>Select GH ID: </b></label>
				   
					<select class='form-control input-lg' name='GHID_new' id='GHID_new' required>
					<option value=''>Please select GH ID</option>
					<?php
					$getGHpending = $con->prepare("SELECT * FROM `gethelp` where (merge='NO' or merge='partial') and user_status='active' order by ghID ASC");
					$getGHpending->execute();
					for($i=1; $i<=rows("SELECT * FROM `gethelp` where (merge='NO' or merge='partial') and user_status='active' order by ghID ASC"); $i++)
					{
						$getGHpending_info = $getGHpending->fetch(PDO::FETCH_ASSOC);
						$pend_ghID = $getGHpending_info['ghID'];
						// $participant_id = $getGHpending_info['pid'];
				   ...
				   <br>
				   <label><b>Enter Amount To GH</b></label>
				   <input class='form-control input-lg' name='amount_toGH'  autocomplete='off' required>
				   <br>
				   <label><b>Enter Balance</b></label>
				   <input class='form-control input-lg' name='amount_blc'  autocomplete='off' required>
				   <br>
					<button class='btn btn-default' name='editGHQueue' id='editGHQueue'>Edit GH Queue &raquo;</button><br><br><br>
				</form>
