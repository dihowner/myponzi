     ...
     <!-- Navigation -->
		...
		<div class="col_1">
			...
			<div class="col-md-12 stats-info">
               <br><font size='5px' color='red'><center><b>.::: Be mindful of how you create orders, do not run down the community :::. </b></center></font>
				<br><div id='result'></div>
			   <br>
			   <form method='post' id='all_PHINFO'>
			   <label><b>Select Participant: </b></label>
			   
				<select class='form-control input-lg' name='participant' id='participant' required>
				<option value=''>Please select a participant</option>
				<?php
				$getparticipant = $con->prepare("SELECT * FROM `participant`");
				...
				<br>
			   <label><b>Amount to Auto-Withdraw: </b></label>
			   
				<select class='form-control input-lg' name='packages' id='packages' required>
				<option value=''>Please select a plan</option>
				<?php
				$getpackages = $con->prepare("SELECT * FROM `packages`");
				$getpackages->execute();
				for($i=1; $i<=rows("SELECT * FROM `packages`"); $i++)
				{
					$getpackages_info = $getpackages->fetch(PDO::FETCH_ASSOC);
					$amount = number_format($getpackages_info['amount']);
					$package_id = $getpackages_info['package_id'];
					$package_name = $getpackages_info['package_name'];
				?>
					<option value='<?php echo $package_id;?>'><?php echo $amount . ' - ' . $package_name;?> </option>
				<?php
				}
				?>
				</select>
			   <br>
				<button class='btn btn-default' name='auto_saveGH' id='auto_saveGH'>Save GH  &raquo;</button><br><br><br>
				</form>
            </div>
