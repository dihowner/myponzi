...
class AutomaticHelper
{
    ...
    public function userPHUnconfirmed($col){
        $result = $this->connection->prepare("SELECT * FROM providehelp WHERE participantID = :col1 AND status = :col2 AND (merge = :col3 OR merge = :col4)");
    ...
    private function getPHByOrderID(){
        try {
			...
			$time = time();
            $res = $this->connection->query("SELECT * FROM providehelp WHERE status = 'Unconfirmed' AND (merge = 'NO' OR merge = 'partial') ORDER BY ID ASC LIMIT 1");
    ...
    private function getResultOfPH($idd,$col){
        $res = $this->connection->prepare("SELECT * FROM providehelp WHERE ID = :phdd");
    ...
    private function updatePHBalance($id, $bal){
        $res = $this->connection->prepare("UPDATE providehelp SET balance = :bal, merge = 'partial' WHERE ID = :phid");
        $res->bindValue(':bal',$bal);
