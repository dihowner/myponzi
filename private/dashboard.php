...
if(isset($username))
{
	...
	$getallForum = $con->prepare("SELECT * FROM `forumtopics` order by topicid desc");
	$getallForum->execute();
	$getallForum_INFO = $getallForum->fetch(PDO::FETCH_ASSOC);
	$topicid = $getallForum_INFO['topicid'];
	// echo $topicid;
	if($check_allpendingGH >=1 && $forum_read == 0 && !empty($topicid))
	{
		redirect_to("forum?viewtopic=$topicid");
	}
	// echo $check_allpendingGH;
                    ...
                    <div class="col-md-12">
						<div class="panel panel-default" style='box-shadow: 0px 0px 2px 0px'>
							<div class='panel-body'>
								<font color='red' size='3'><b>OUR TESTIMONY:</b></font> <a href='testimonial' class='btn btn-default'><b>READ MORE</b></a>
								<br/><br/>
								<marquee direction='up' height='50px' scrolldelay='250'>
								<?php
								$getall_testimony = $con->prepare("select * from `testimonies` order by `testimonial_id` desc");
								...
								for($i=1; $i<=rows("select * from `testimonies` order by `testimonial_id` desc"); $i++)
								{
									...
									{
										echo nl2br(strtoupper($gh_letter)) . '<hr>';
									}
								}
								?>
								</marquee>              
							...
							<div class="panel panel-default" style=''>
									...
									<div class="panel-body">
										...
										for($i=1; $i<=$checkallGH_row_topay; $i++)
										{
											...
											//Trust betrayed
											$additional_time =  date('M d, Y H:i:00', strtotime("$dateMerge_expires+2 hours"));
							...
							?>
								...
								?>
									...
									<div class='col-md-7'>
										...
										<?php 
										if(empty($attachment) && $merge_status_pay =='')
										{?>
										<a href='../action?EXTENDTIME&ID=<?php echo $ghID;?>&mergeID=<?php echo $mergeID;?>' class='btn btn-default' onclick="return confirm('Add Additional 6hours?');"><b><i class='fa fa-plus'></i> EXTEND TIME</b></a>
											
										<?php
										}
										else if(!empty($attachment) && $merge_status_pay =='Upload') 
										{?>
